  ...
  return (
    <div className="flex flex-col h-screen bg-background font-sans">
      <header className="p-4 border-b">
        <h1 className="text-xl font-bold text-foreground">Asesor IA (Modo Estable HTTP)</h1>
      </header>
      ...
      <footer className="p-4 border-t bg-background">
        <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto flex items-center gap-4">
          <input
            ...
            className="flex-1 p-3 border rounded-lg focus:ring-2 focus:ring-ring focus:outline-none bg-input text-foreground"
            disabled={status === 'loading'}
          />
          <button
            type="submit"
            className="p-3 w-12 h-12 bg-primary text-primary-foreground rounded-full flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed hover:bg-primary/90 transition-colors"
